...
<div class="form-container" style="max-width: 900px; width: 95%;">
    <div class="glass-card mb-4">
        <h2 class="title-main text-center">Pending Payments</h2>
        <p class="subtitle text-center">Complete payment for your confirmed bookings.</p>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 2rem;">
            <thead>
                <tr style="border-bottom: 2px solid var(--primary-color);">
                    ...
                    <th style="padding: 1rem; text-align: right;">Total Price</th>
                </tr>
            </thead>
            <tbody>
                <?php
                ...
                $stmt = $conn->prepare("SELECT book_id, name, room_type, checkin, checkout, price FROM confirmed_booking WHERE phone = ? AND idproof = ?");
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td style='padding: 1rem; text-align: right; font-weight: bold; color: var(--primary-color);'>$" . htmlspecialchars($row['price']) . "</td>";
                        echo "</tr>";
                    }
                } else {
                    echo "<tr><td colspan='6' style='text-align: center; padding: 2rem;'>No confirmed bookings requiring payment.</td></tr>";
                }
                $stmt->close();
                ?>
            </tbody>
    ...
    <div class="glass-card">
        <h3 class="text-center mb-4" style="color: var(--secondary-color);">Make a Payment</h3>
        <form action="payment.php" method="post" class="form-row" style="align-items: flex-end;">
